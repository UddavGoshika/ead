...
router.post('/', async (req, res) => {
    try {
        ...
        // NOTIFICATION & EMAIL (Wrapped to avoid failing the whole request)
        try {
            ...
            const emailHtml = `<h3>New Contact Inquiry</h3><p><strong>Name:</strong> ${name}</p><p><strong>Email:</strong> ${email}</p><p><strong>Phone:</strong> ${phone}</p><p><strong>Message:</strong></p><p>${message}</p>`;

            const { sendEmail } = require('../utils/mailer');
            await sendEmail(adminEmail, `New Inquiry from ${name}`, `New inquiry from ${name}: ${message}`, emailHtml);
        } catch (notifErr) {
            console.error("Non-critical contact side-effect failed:", notifErr);
        }
