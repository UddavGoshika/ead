...
async function check() {
    try {
        ...
        const activeAdvocates = await Advocate.find().populate('userId', 'status role');
        const statusCounts = {};
        activeAdvocates.forEach(a => {
            const s = a.userId?.status || 'No User';
            statusCounts[s] = (statusCounts[s] || 0) + 1;
        });
        console.log('Status counts for all Advocate profiles:', statusCounts);
