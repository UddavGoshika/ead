...
import styles from './AdvisorChatPopup.module.css';
import {
    ChevronLeft,
    MoreVertical,
    Phone,
...
const AdvisorChatPopup: React.FC<ChatPopupProps> = ({ advocate: initialAdvocate, onClose, onSent }) => {
    ...
    const rawName = advocate.name || (advocate.firstName ? `${advocate.firstName} ${advocate.lastName}` : "User");
    const rawId = advocate.unique_id || advocate.partnerUniqueId || "N/A";
    const displayName = shouldMask && rawName ? (rawName.substring(0, 2) + '*****') : (rawName || "User");
    const displayId = shouldMask ? (rawId || "---").substring(0, 2) + '*****' : (rawId || "---");
            ...
            <div className={styles.popup} onClick={e => e.stopPropagation()}>
                <header className={styles.header}>
                    ...
                    <div className={styles.profileClickArea} onClick={() => setShowDetailedProfile(true)}>
                        <img src={formatImageUrl(displayImage)} alt={displayName} className={`${styles.avatar} ${shouldMask ? styles.blurredAvatar : ''}`} />
...
export default AdvisorChatPopup;
